...
/**
 * Local types for Recharts tooltip/legend payload items.
 * Recharts 3.x exposes payload as `unknown` in types; we'll define a safe shape.
 */
...
function ChartTooltipContent({
  ...
  active?: boolean
  // payload is unknown in Recharts types — accept unknown and coerce safely
  payload?: unknown
  className?: string
  ...
  // Safely coerce payload to an array of TooltipPayloadItem
    ...
    >
      ...
      <div className="grid gap-1.5">
        {items.map((item, index) => {
          ...
          const itemConfig = getPayloadConfigFromPayload(config, item, key)
          // prefer explicit color argument, then payload.fill, then item.color
          const indicatorColor = color || (item.payload && item.payload.fill) || item.color || item.fill
...
function ChartLegendContent({
  ...
  className?: string
  hideIcon?: boolean
  // legend payload from recharts is unknown typed; coerce safely
  payload?: unknown
          ...
          >
            {itemConfig?.icon && !hideIcon ? (
              ...
              <div
                ...
                style={{
                  backgroundColor: (item.color ?? (item.payload && item.payload.fill)) as string | undefined,
                }}
              />
            )}
            {itemConfig?.label}
...
// Accepts payload of unknown shape, returns matched config entry or undefined.
function getPayloadConfigFromPayload(
  ...
  // payload.payload can exist: try to normalize
